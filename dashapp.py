...
NpArrayBool = np.typing.NDArray[bool]
AkArray = awkward.Array

# --- Configuration ---
config = SimpleNamespace(
    PLOT_EXTENT=5000,
    ROOT_FILE_PATH="data/p8_ee_tt_ecm380/reco_p8_ee_tt_ecm380_1302.root",
    PARQUET_FILE_PATH="data/p8_ee_tt_ecm380/reco_p8_ee_tt_ecm380_1302.parquet",
    ...
    PANDORA_CLUSTER_FEATURES=["type", "energy", "energyError", "position.x", "position.y", "position.z"],
...
def pad_and_concatenate_arrays(arr1: NpArrayFloat, arr2: NpArrayFloat) -> NpArrayFloat:
    if arr1.ndim != 2 and arr1.shape[0] != 0:
        arr1 = arr1.reshape(0, 0)
    if arr2.ndim != 2 and arr2.shape[0] != 0:
        arr2 = arr2.reshape(0, 0)
...
def load_root_data(file_path: str) -> LoadedRootData:
    ...
    try:
        ...
        ev_tree = fi.get("events")
        metadata_tree = fi.get("metadata")
        if metadata_tree:
            collectionIDs_data = metadata_tree.arrays("CollectionIDs")
            if (
                "CollectionIDs" in collectionIDs_data.fields
                and "m_names" in collectionIDs_data["CollectionIDs"].fields
                and "m_collectionIDs" in collectionIDs_data["CollectionIDs"].fields
                and len(collectionIDs_data["CollectionIDs"]["m_names"]) > 0
            ):
        ...
        print(f"ERROR: ROOT file not found at {file_path}.")
        return None, None, {}, {}, 0
...
def load_mlpf_model(kwargs_path: str, checkpoint_path: str, device: torch.device) -> LoadedMLPFModel:
    ...
    try:
        ...
        # remove PU weight from model
        if "nn_pu.4.weight" in model_state:
            model_state.pop("nn_pu.4.weight")
            model_state.pop("nn_pu.4.bias")
...
def extract_hit_features(event_tree: uproot.TTree, event_index: int, collection_name: str, features_to_extract: List[str]) -> AkArray:
    ...
    try:
        event_collection_data = event_tree.arrays(branches_to_load, entry_start=event_index, entry_stop=event_index + 1, library="ak")
    except Exception:
        return awkward.Array({f_orig: awkward.Array([]) for f_orig, _ in feature_map} | {"subdetector": awkward.Array([])})
    found_data_for_event = any(
        bnk in event_collection_data.fields and len(event_collection_data[bnk][0]) > 0
        for bnk in branches_to_load
        if bnk in event_collection_data.fields
    )
    if not found_data_for_event:
        return awkward.Array({f_orig: awkward.Array([]) for f_orig, _ in feature_map} | {"subdetector": awkward.Array([])})
    num_hits = 0
    first_branch_key = f"{collection_name}.{feature_map[0][1]}"
    if first_branch_key in event_collection_data.fields and len(event_collection_data[first_branch_key]) > 0:
    ...
    for original_feat_name, feat_name_in_file in feature_map:
        ...
        if branch_full_name_key in event_collection_data.fields and len(event_collection_data[branch_full_name_key]) > 0:
            ...
            feat_arr_for_event[original_feat_name] = awkward.from_numpy(np.zeros(num_hits))
    subdetector_val = 3
    if collection_name.startswith("ECAL"):
        subdetector_val = 0
    elif collection_name.startswith("HCAL"):
        subdetector_val = 1
    elif collection_name.startswith("MUON"):
        subdetector_val = 2
...
def extract_track_features(
    event_tree: uproot.TTree, event_index: int, track_collection: str, track_state_branch: str, b_field_tesla: float
) -> TrackFeatures:
    ...
    track_branches.append(f"{track_collection}/{track_collection}.trackStates_begin")
    ...
    if any(b.startswith(f"{track_collection}.") and b not in available_branches for b in track_branches) or any(
        b not in available_branches for b in state_branches
    ):
        return empty_result
    try:
        event_track_data = event_tree.arrays(all_needed_branches, entry_start=event_index, entry_stop=event_index + 1, library="ak")
    except Exception:
        return empty_result
    ...
    if len(track_state_indices) != num_tracks or (len(track_state_indices) > 0 and awkward.any(track_state_indices < 0)):
        return empty_result
    for param_name in track_state_params:
        ...
        if len(track_state_indices) == 0:
            track_features_ak[param_name] = awkward.Array([])
        elif awkward.any(track_state_indices >= len(all_states_for_param)):
            return empty_result
        else:
...
def extract_pandora_cluster_features(
    event_tree: uproot.TTree, event_index: int, collection_name: str, features_to_extract: List[str]
    ...
    if not available_branches_in_tree:
        return None
    try:
        pandora_event_data = event_tree.arrays(available_branches_in_tree, entry_start=event_index, entry_stop=event_index + 1, library="ak")
    except Exception:
        return None
    num_clusters = 0
    ...
    for original_feat_name in features_to_extract:
        ...
        df_col_name = pandora_feature_to_df_col_map.get(original_feat_name, original_feat_name)
...
MLPFInferenceResult = Tuple[
    Optional[MLPFOutput], str, List[Tuple[str, NpArrayFloat]], List[Tuple[str, NpArrayFloat]]
]  # preds, msg, attentions, embeddings


def run_mlpf_inference(
    model: MLPF, model_kwargs: Dict[str, Any], x_track_event_np: NpArrayFloat, x_cluster_event_np: NpArrayFloat, device: torch.device
) -> MLPFInferenceResult:
    ...
    if hasattr(model, "conv_id") and model.conv_id is not None and len(model.conv_id) > 0:
        ...
        hook = model.conv_id[-1].register_forward_hook(functools.partial(embedding_capture_hook, "final_conv_id_output"))
        hooks.append(hook)
    ...
    if hasattr(model, "conv_reg") and model.conv_reg is not None and len(model.conv_reg) > 0:
        ...
        hook = model.conv_reg[-1].register_forward_hook(functools.partial(embedding_capture_hook, "final_conv_reg_output"))
        hooks.append(hook)
    # --- End of Hook Setup ---
    ...
    try:
        with torch.no_grad():
            ...
            if isinstance(raw_model_output, tuple) and len(raw_model_output) == 4 and all(isinstance(p, torch.Tensor) for p in raw_model_output):
...
def create_feature_heatmap(data_np: NpArrayFloat, feature_order: List[str], title: str, y_axis_prefix: str, colorscale: str = "Viridis") -> go.Figure:
    ...
    if data_np.shape[1] != len(feature_order):
        return create_empty_heatmap_figure(f"{title} - Feature Mismatch (Data: {data_np.shape[1]}, Expected: {len(feature_order)})")
    fig = go.Figure(
        data=go.Heatmap(
            z=data_np,
            x=feature_order,
    ...
    fig.update_layout(
        title=title, xaxis_title="Features", yaxis_title=f"{y_axis_prefix} Index", xaxis_tickangle=-45, margin=dict(l=70, r=20, b=120, t=50)
    )
    return fig


def create_attention_heatmap_figure(attn_matrix_np: NpArrayFloat, title: str, colorscale: str = "Blues") -> go.Figure:
    if not isinstance(attn_matrix_np, np.ndarray) or attn_matrix_np.ndim != 2 or attn_matrix_np.shape[0] == 0 or attn_matrix_np.shape[1] == 0:
        fig = go.Figure().update_layout(
            title=f"{title} - No/Invalid Data (Shape: {attn_matrix_np.shape if isinstance(attn_matrix_np, np.ndarray) else 'N/A'})"
        )
    ...
    fig = go.Figure(
        data=go.Heatmap(z=attn_matrix_np, x=x_labels, y=y_labels, colorscale=colorscale, colorbar=dict(title="Attention Weight"), xgap=0, ygap=0)
    )
    fig.update_layout(
        title=title,
...
def create_embedding_heatmap_figure(embedding_matrix_np: NpArrayFloat, title: str, colorscale: str = "RdBu") -> go.Figure:
    """Creates a heatmap figure for an embedding matrix."""
    if (
        not isinstance(embedding_matrix_np, np.ndarray)
        or embedding_matrix_np.ndim != 2
        or embedding_matrix_np.shape[0] == 0
        or embedding_matrix_np.shape[1] == 0
    ):
        fig = go.Figure().update_layout(
            title=f"{title} - No/Invalid Data (Shape: {embedding_matrix_np.shape if isinstance(embedding_matrix_np, np.ndarray) else 'N/A'})"
        )
...
def generate_helix_trace_data(
    px_values: NpArrayFloat,
    py_values: NpArrayFloat,
    pz_values: NpArrayFloat,
    charge_values: NpArrayInt,
    particle_mass: float,
    b_field: float,
    scale_factor: float,
    num_points_helix: int = 70,
    num_points_straight: int = 50,
) -> HelixTraceData:
    ...
    for i in range(len(px_values)):
        ...
        if charge_values[i] == 0 or abs(b_field) < 1e-9 or particle_vec.pt < 1e-6:
            ...
            radius_m = particle_vec.pt / (charge_values[i] * 0.299792458 * b_field)
            time_vals = np.linspace(0, path_len_m / (config.C_LIGHT * beta), num_points_helix)
            signed_arg_angle_helix = config.C_LIGHT * charge_values[i] * 0.299792458 * b_field / (particle_vec.gamma * particle_vec.mass)
            initial_phase_angle = particle_vec.phi - np.pi / 2.0
            path_x = scale_factor * radius_m * (np.cos(initial_phase_angle + signed_arg_angle_helix * time_vals) - np.cos(initial_phase_angle))
            path_y = scale_factor * radius_m * (np.sin(initial_phase_angle + signed_arg_angle_helix * time_vals) - np.sin(initial_phase_angle))
...
def create_3d_event_display_figure(
    traces: List[go.Scatter3d], event_index: int, b_field: float, scale_factor: float
) -> go.Figure:
    fig = go.Figure(data=traces)
    title = f"3D Event Display: Event {event_index} (B={b_field:.1f}T, Scale={scale_factor:.0f})"
    fig.update_layout(
        title=title,
        scene=dict(
            xaxis=dict(
                ...
                showbackground=True,
                zerolinecolor="white",
                range=[-config.PLOT_EXTENT, config.PLOT_EXTENT],
            ),
            yaxis=dict(
                title="Y (mm)",
                backgroundcolor="rgb(230, 230,230)",
        ...
        uirevision="persistent_view",
    )
    fig.update_traces(marker_line_width=0, selector=dict(type="scatter3d"))
    if not traces:
        fig.add_annotation(
            text="No 3D data to display for this event or parameters.", xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False, font_size=16
        )
    return fig
...
def initialize_global_data() -> None:
    global root_file_obj, event_tree_global, collectionIDs_global, collectionIDs_reverse_global
    global max_events_root_global, parquet_data_global, max_events_pq_global, mlpf_model_global
    global model_kwargs_global, MAX_EVENTS_AVAILABLE
    print(f"Using PyTorch device: {config.TORCH_DEVICE}")
    root_file_obj, event_tree_global, collectionIDs_global, collectionIDs_reverse_global, max_events_root_global = load_root_data(
    ...
    assert max_events_pq_global == max_events_root_global
    MAX_EVENTS_AVAILABLE = max_events_pq_global


# --- Dash Application ---
app = dash.Dash(__name__)
app.title = "CLIC Event Display & Feature Inspector"
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                            ...
                            html.Div(
                                ...
                                children=[
                                    ...
                                    dcc.Input(
                                        id="iev-input",
                                        type="number",
                                        value=config.DEFAULT_EVENT_INDEX,
                                        min=0,
                                        max=max_event_val,
                                        step=1,
                            ...
                            html.Div(
                                ...
                                children=[
                                    ...
                                    dcc.Input(
                                        id="b-field-input",
                                        type="number",
                                        value=config.DEFAULT_B_FIELD_TESLA,
                                        step=0.1,
                                        style={"padding": "5px", "width": "80px"},
                                    ),
                                ],
                            ...
                            html.Div(
                                ...
                                children=[
                                    ...
                                    dcc.Input(
                                        id="scale-input",
                                        type="number",
                                        value=config.DEFAULT_SCALE_FACTOR,
                                        step=100,
                                        style={"padding": "5px", "width": "80px"},
                                    ),
                                ],
                    ...
                    html.Div(
                        id="model-inference-output-display",
                        style={
                            "marginTop": "10px",
                            "marginBottom": "10px",
                            "color": "blue",
                    ...
                    html.Div(
                        style={"display": "flex", "flexDirection": "row", "justifyContent": "space-between", "gap": "15px", "flexWrap": "wrap"},
                        children=[
                            html.Div(
                                style={"flex": "2 1 600px", "minWidth": "60%"},
                                children=[
                                    dcc.Loading(
                                        id="loading-graph",
                                        type="circle",
                                        children=[dcc.Graph(id="particle-graph-display", style={"height": "75vh", "minHeight": "500px"})],
                                    )
                                ],
                            ),
                            html.Div(
                                style={"flex": "1 1 300px", "minWidth": "35%", "display": "flex", "flexDirection": "column", "gap": "15px"},
                                children=[
                                    html.H3("Track Features (X_track) - Normalized", style={"textAlign": "center", "marginBlock": "5px"}),
                                    dcc.Loading(
                                        id="loading-xtrack-heatmap",
                                        type="circle",
                                        children=[dcc.Graph(id="xtrack-heatmap-display", style={"height": "32vh", "minHeight": "220px"})],
                                    ),
                                    html.H3("Cluster Features (X_cluster) - Normalized", style={"textAlign": "center", "marginBlock": "5px"}),
                                    dcc.Loading(
                                        id="loading-xcluster-heatmap",
                                        type="circle",
                                        children=[dcc.Graph(id="xcluster-heatmap-display", style={"height": "32vh", "minHeight": "220px"})],
                                    ),
                                ],
                            ),
                        ],
...
@app.callback(
    ...
    [Input("iev-input", "value"), Input("b-field-input", "value"), Input("scale-input", "value"), Input("display-toggle-checklist", "value")],
)
def update_graph_and_heatmaps(
    current_event_idx: Optional[int], b_field_val: Optional[float], scale_factor_val: Optional[float], display_options: List[str]
) -> Tuple[go.Figure, go.Figure, go.Figure, str, str, List[Any], List[Any]]:
    print(
        f"Callback triggered. Event Index: {current_event_idx}, B-Field: {b_field_val}, Scale: {scale_factor_val}, Display Options: {display_options}"
    )
    ...
    if current_event_idx is None:
        current_event_idx = config.DEFAULT_EVENT_INDEX
    if b_field_val is None:
        b_field_val = config.DEFAULT_B_FIELD_TESLA
    if scale_factor_val is None:
        scale_factor_val = config.DEFAULT_SCALE_FACTOR
    ...
    fig_3d = go.Figure().update_layout(
        title="Error: Event index invalid or data unavailable.", scene=dict(xaxis_title="X (mm)", yaxis_title="Y (mm)", zaxis_title="Z (mm)")
    )
    ...
    if (
        mlpf_model_global
        and model_kwargs_global
        and not (parquet_data_global is None)
        and "X_track" in parquet_data_global.fields
        and "X_cluster" in parquet_data_global.fields
        and 0 <= current_event_idx < max_events_pq_global
    ):
        try:
            ...
            if x_track_event_np.ndim < 2 and x_track_event_np.shape != (0, 0):
                x_track_event_np = np.atleast_2d(x_track_event_np)
            if x_cluster_event_np.ndim < 2 and x_cluster_event_np.shape != (0, 0):
                x_cluster_event_np = np.atleast_2d(x_cluster_event_np)
            ...
            if mlpf_predictions is None:
                error_messages.append(f"MLPF Inference: {inference_msg}")
        except Exception as e_infer:
            error_messages.append(f"Error during MLPF model inference: {str(e_infer)}")
    ...
    if attention_data:
        ...
        if conv_id_attentions:
            ...
            attention_figures_children.append(
                html.H4("Convolutional Identification Layers (conv_id)", style={"textAlign": "center", "marginTop": "15px", "marginBottom": "5px"})
            )
            row_children = []
        ...
        if conv_reg_attentions:
            processed_any_attention_group = True
            attention_figures_children.append(
                html.H4("Convolutional Regression Layers (conv_reg)", style={"textAlign": "center", "marginTop": "15px", "marginBottom": "5px"})
            )
            row_children = []
    ...
    if embedding_data:
        embedding_figures_children.append(
            html.H3("Intermediate Embedding Heatmaps", style={"textAlign": "center", "marginTop": "20px", "marginBottom": "10px"})
        )
        ...
        for name, emb_matrix in embedding_data:
            ...
            embedding_row_children.append(html.Div([dcc.Graph(figure=emb_fig, style=graph_style)], style=container_style))
        embedding_figures_children.append(
            html.Div(
                embedding_row_children,
                style={
        ...
        embedding_figures_children.append(
            html.P("No intermediate embedding data captured or available for this event.", style={"textAlign": "center"})
        )
            ...
            if "X_track" in parquet_data_global.fields and len(parquet_data_global["X_track"]) > current_event_idx:
                ...
                if len(x_track_data_ak) > 0:
                    ...
                    if x_track_np_hm.ndim > 0 and x_track_np_hm.shape[0] > 0:
                        ...
                        stds_trk[stds_trk == 0] = 1
                        x_track_np_norm = (x_track_np_hm - means_trk) / stds_trk
                        x_track_np_norm[np.isnan(x_track_np_norm)] = 0
                        xtrack_fig = create_feature_heatmap(
                            x_track_np_norm, config.TRACK_FEATURE_ORDER, f"X_track (Event {current_event_idx})", "Trk"
                        )
                    else:
                        xtrack_fig = create_feature_heatmap(
                            np.array([]), config.TRACK_FEATURE_ORDER, f"X_track (Event {current_event_idx}) - No Tracks", "Trk"
                        )
                else:
                    xtrack_fig = create_feature_heatmap(
                        np.array([]), config.TRACK_FEATURE_ORDER, f"X_track (Event {current_event_idx}) - No Tracks", "Trk"
                ...
                error_messages.append(f"X_track data not found for event {current_event_idx} in Parquet.")
            if "X_cluster" in parquet_data_global.fields and len(parquet_data_global["X_cluster"]) > current_event_idx:
                ...
                if len(x_cluster_data_ak) > 0:
                    ...
                    if x_cluster_np_hm.ndim > 0 and x_cluster_np_hm.shape[0] > 0:
                        ...
                        xcluster_fig = create_feature_heatmap(
                            x_cluster_np_norm, config.CLUSTER_FEATURE_ORDER, f"X_cluster (Event {current_event_idx})", "Cls", colorscale="Plasma"
                        )
                    else:
                        xcluster_fig = create_feature_heatmap(
                            np.array([]),
                            config.CLUSTER_FEATURE_ORDER,
                            f"X_cluster (Event {current_event_idx}) - No Clusters",
                            "Cls",
                            colorscale="Plasma",
                        )
                else:
                    xcluster_fig = create_feature_heatmap(
                        np.array([]), config.CLUSTER_FEATURE_ORDER, f"X_cluster (Event {current_event_idx}) - No Clusters", "Cls", colorscale="Plasma"
                    )
            else:
                error_messages.append(f"X_cluster data not found for event {current_event_idx} in Parquet.")
    ...
    if show_hits:
        ...
        if len(hit_feature_matrix) > 0 and all(
            f in hit_feature_matrix.fields for f in ["position.x", "position.y", "position.z", "energy", "subdetector"]
        ):
            ...
            df_hits["energy"] = 1000 * df_hits["energy"]
            df_hits["plotsize"] = 2.0
            df_hits.loc[df_hits["subdetector"] == 0, "plotsize"] = np.clip(
                2 + 2 * np.log1p(df_hits.loc[df_hits["subdetector"] == 0, "energy"] / 5), 1, 10
            )
            df_hits.loc[df_hits["subdetector"] == 1, "plotsize"] = np.clip(
                2 + 2 * np.log1p(df_hits.loc[df_hits["subdetector"] == 1, "energy"] / 10), 1, 10
            )
            df_hits.loc[df_hits["subdetector"] == 2, "plotsize"] = np.clip(
                2 + 2 * np.log1p(df_hits.loc[df_hits["subdetector"] == 2, "energy"] * 100), 1, 10
            )
            ...
            for subdet_idx in df_hits["subdetector"].unique():
                ...
                fig_traces_3d.append(
                    go.Scatter3d(
                        x=np.clip(sub_df["px"], -config.PLOT_EXTENT, config.PLOT_EXTENT),
                        y=np.clip(sub_df["py"], -config.PLOT_EXTENT, config.PLOT_EXTENT),
                        z=np.clip(sub_df["pz"], -config.PLOT_EXTENT, config.PLOT_EXTENT),
                        mode="markers",
    ...
    if show_tracks:
        track_px_all, track_py_all, track_pz_all, track_charge_all, track_pt_all = extract_track_features(
            event_tree_global, current_event_idx, config.DEFAULT_TRACK_COLLECTION_NAME, config.DEFAULT_TRACK_STATE_BRANCH_NAME, b_field_val
        )
        if track_pt_all.size > 0:
            ...
            if track_px.size > 0:
                hx, hy, hz = generate_helix_trace_data(
                    track_px, track_py, track_pz, track_charge, config.PION_MASS_GEV, b_field_val, scale_factor_val
                )
                fig_traces_3d.append(go.Scatter3d(x=hx, y=hy, z=hz, mode="lines", line=dict(color="purple", width=3), name="SiTracks"))
    ...
    if show_pandora_clusters:
        df_pandora = extract_pandora_cluster_features(
            event_tree_global, current_event_idx, config.PANDORA_CLUSTER_COLLECTION_NAME, config.PANDORA_CLUSTER_FEATURES
        )
        if df_pandora is not None and not df_pandora.empty:
            if all(col in df_pandora for col in ["x", "y", "z", "energy"]):
                ...
                hover_texts_pandora = [
                    ...
                    + (
                        f"<br>E_err: {df_pandora['energyError'][i]:.2f} GeV"
                        if "energyError" in df_pandora and pd.notna(df_pandora["energyError"][i])
                        else ""
                    )
                ...
                fig_traces_3d.append(
                    go.Scatter3d(
                        ...
                        name="Pandora Clusters",
                        text=hover_texts_pandora,
                        hoverinfo="text+name",
                    )
                )
            else:
                error_messages.append("Essential Pandora Cluster data (pos/energy) missing.")
    ...
    fig_3d = create_3d_event_display_figure(
        fig_traces_3d, current_event_idx, b_field_val, scale_factor_val
    )
